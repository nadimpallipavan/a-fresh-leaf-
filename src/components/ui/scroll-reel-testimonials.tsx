...
const AUTHOR_CLASSES =
  "m-0 text-sm font-medium leading-[1.3] text-leaf-400";
...
function Cell() {
  return (
    <div
      aria-hidden="true"
      className="shrink-0 rounded-xl border border-leaf-900/60 bg-gradient-to-b from-leaf-950/40 to-leaf-900/20 blur-[1px] shadow-[0_1px_2px_rgba(0,0,0,0.05)] dark:shadow-[0_1px_2px_rgba(0,0,0,0.3)]"
...
function Featured({ src, alt }: { src: string; alt?: string }) {
  return (
    <div
      className="relative shrink-0 overflow-hidden rounded-xl bg-leaf-900 border border-leaf-700"
    ...
    >
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(220.99deg, rgba(90,200,120,0) 32%, rgb(90,200,120) 41%, rgb(173,177,255) 47%, rgba(130,189,237,0.57) 54%, rgba(130,189,237,0) 65%)",
        }}
      />
    </div>
  );
...
export function ScrollReelTestimonials({
  ...
  return (
    <div
      ...
      onKeyDown={onKeyDown}
      className={cn(
        "relative flex w-full max-w-[1060px] flex-col items-stretch gap-2.5 overflow-hidden rounded-xl border border-leaf-800 bg-leaf-950/40 backdrop-blur-md outline-none focus-visible:ring-2 focus-visible:ring-leaf-500 md:min-h-[320px] md:flex-row",
        className
      )}
    >
      {/* Reel section */}
      ...
      <div className="flex min-w-0 flex-1 flex-col justify-between self-stretch px-5 py-7 md:py-10">
        <div className="flex flex-col gap-[9px]">
          <svg
            className="block h-12 w-12 text-leaf-500/40"
            viewBox="0 0 24 24"
            fill="currentColor"
            aria-hidden="true"
          >
        ...
        <div className="mt-6 flex items-center gap-1.5 md:mt-0">
          <button
            ...
            className="grid h-6 w-6 cursor-pointer place-items-center rounded-full border border-leaf-500/30 bg-transparent p-0 text-leaf-300 transition-[opacity,transform] duration-200 ease-[cubic-bezier(0.22,1,0.36,1)] hover:enabled:scale-[1.08] active:enabled:scale-[0.94] disabled:cursor-default disabled:opacity-40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-leaf-500"
          >
            <svg
